...
if (empty($obj->get_idabsen($userid))) {
	...
	<script>
		swal.fire({
			title: "Gagal!",
			html: "Anda tidak berada pada lokasi<br>Sekolahan!<br><br>Jarak Anda: ' . $jarak . ' km",
			icon: "error",
		}).then((result) => {
			setTimeout(function () {
				window.location.href = "login";
...
<script>
    ...
    // Ambil elemen tombol "ABSEN MASUK"
    const btnAbsenMasuk = document.querySelector(".button-title"); // Tombol "ABSEN MASUK" dengan class .button-title
    
    // Ambil elemen tombol "ABSEN KELUAR"
    const absenKeluarItem = document.getElementById("absenKeluarItem");
    
    // Awalnya sembunyikan tombol "ABSEN KELUAR"
    absenKeluarItem.style.display = "none";  // Tombol "ABSEN KELUAR" disembunyikan
    
    // Event listener untuk tombol "ABSEN MASUK"
    btnAbsenMasuk.addEventListener("click", function () {
        // Setelah tombol "ABSEN MASUK" diklik, tampilkan tombol "ABSEN KELUAR"
        absenKeluarItem.style.display = "block";  // Tombol "ABSEN KELUAR" muncul
    });
});
